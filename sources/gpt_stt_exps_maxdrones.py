...
class GPTInterface(LLMInterface):
    ...
    :param model_name: The name of the GPT model to use. Defaults to "gpt-4o".
    """
    ...
    def process_input(self, text: str, image_path: Optional[str] = None) -> str:
        ...
        :param image_path: The file path to an image to include in the request (optional).
        ...
        messages = [
            HumanMessage(
                content=[
                    {"type": "text", "text": text}  # Text input for the model
                ]
            )
        ]

        # If an image path is provided, encode the image and add it to the message
        if image_path:
...
experiment_types = {
    ...
    'lines': 'Generate a set of parallel lines using 100 drones. Each drone should follow a separate line, ensuring that '
...
def interpret_text_request(prompt: str) -> str:
    ...
    # model = genai.GenerativeModel('models/vlm-1.5-flash')
    # response = model.generate_content([base_prompt, prompt])
    response = call_vlm_with_retry(prompt=f"{base_prompt}\n\n{prompt}")
...
def fetch_waypoints_code_from_vlm(requirements: str, error: str = None):
    """
    ...
    response = call_vlm_with_retry(prompt=base_prompt)
    code_text = None
    try:
        code_text = response
    except ValueError as e:
...
def analyze_plot_with_multiple_critics(
    ...
    for i in range(num_critics):
        ...
        response = call_vlm_with_retry(prompt=base_prompt, image_path=image_path)
        try:
            feedback = response
...
def aggregate_feedback(
    ...
    summary = response if response else "Error in generating summary"
...
def process_waypoints_with_retry(
    ...
    for attempt in range(max_retries):
        code_response = fetch_waypoints_code_from_vlm(requirements, error or feedback)
        try:
...
def call_vlm_with_retry(prompt: str, image_path: Optional[str] = None, model_name: str = 'gpt-4o',
                        max_attempts: int = 5, base_delay: float = 0.1) -> Optional[str]:
    """
    Calls the GPT-4 model API with retry logic and exponential backoff.

    :param prompt: The prompt to send to the model.
    :param image_path: Optional path to an image file to include with the prompt.
    ...
    """
    gpt_interface = GPTInterface(model_name=model_name)

    for attempt in range(max_attempts):
        try:
            response = gpt_interface.process_input(prompt, image_path)
            return response  # Return the response text if successful
        except Exception as e:
            logging.error(f"An error occurred during GPT API call: {e}")
            if "429" in str(e):
    ...
    logging.error("Failed to get a valid response from the GPT API.")
...
def main():
    ...
    for experiment_type, experiment_prompt in experiment_types.items():
        ...
        experiment_type_dir = os.path.abspath(os.path.join("ablations_maxdrones_gpt", timestamped_experiment_type))
